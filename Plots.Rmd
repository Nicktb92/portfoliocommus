...
graph <- newggslopegraph(CompMatMeans.graph, 
                         ...
                         Value, 
                         Feature, 
                         LineColor = c("danceability" = "#ff9a00", "energy" = "#ff9a00",
                                       "speechiness" = "grey", "acousticness" = "#ff9a00",
                                       "instrumentalness" = "#ea0909", "liveness" = "grey", 
                                       "valence" = "#ea0909"),
                         LineThickness = 0.5) +
    labs(title="Audio Features", 
         subtitle="Change of Means", 
         caption="Red: > 0.5 Standard Deviation\nOrange: 0.2 - 0.5 Standard Deviation\nGrey: < 0.2 Standard Deviation", x = NULL) +
    theme_minimal()
...
SNfull$group[306:406] <- rep("SNnew", 100)
SNfull$group[1:100] <- rep("SNold", 100)
SNfull$group[101:305] <- rep(NA, 100)
...
```{r}
SN_group <- SN_long[c(1:100, 306:406),]

par(mfrow = c(2,2))

colnames(SN_group) <- make.unique(names(SN_group))

plot2 <- na.omit(SN_group) %>%
  ...
  xlab("Danceability") +
  ylab("Valence") +
  labs(title = "Density Plot", subtitle = "Development of Playlist over Time", colour = "Key Mode") 
...
library(dygraphs)
dygraph(SN_long, main = "New Haven Temperatures") %>% 
...
na.omit(SN_group) %>%
  ggplot(aes(x = loudness, y = track.popularity)) +
  facet_grid(rows = vars(group)) +
  stat_density2d(aes(fill = ..density..), contour = F, geom = 'tile') +
  scale_fill_distiller(palette = 'BuPu') +
  geom_point(alpha = 0.9, aes(col = mode_name)) +
  theme_minimal() +
    ...
    scale_x_continuous(          # Fine-tune the x axis.
        limits = c(-30, 0),
        breaks = c(-30, -15, 0),  # Use grid-lines for quadrants only.
  ...
  labs(title = "Density Plot", subtitle = "Development of Music Taste over Time", colour = "Key Mode") 
...
```{r}
ggplot(monthdivision[1:10,], aes(y = abs(valencediff), x = as.Date(Group.1))) +
  geom_area(col = "#88419D", fill = "#88419D", alpha = 0.6) +
